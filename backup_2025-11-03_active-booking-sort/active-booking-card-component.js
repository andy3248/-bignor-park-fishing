...
(function() {
    ...
    function getAllUserBookings(userId) {
        ...
        if (stored) {
            try {
                ...
                console.error('[BookingCard] Error reading legacy bookings:', error);
            }
        }
    ...
    // Render single booking card HTML
    function renderSingleBookingCard(booking) {
        ...
        return `
            <div class="booking-info-card" data-booking-id="${escapeHtml(booking.id)}" style="margin-bottom: 20px;">
    ...
    // Render all bookings for a user (sorted by date, nearest first)
    function renderBookingCard(userId, containerElement) {
        if (!containerElement) {
        ...
        const cardsHTML = activeBookings.map(booking => renderSingleBookingCard(booking)).join('');
