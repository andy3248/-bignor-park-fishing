...
// Get the ActiveBooking system functions (safely)
...
// Helper functions to safely access ActiveBookingSystem
...
const getActiveBooking = (...args) => getActiveBookingSystem()?.getActiveBooking(...args);
const clearActiveBooking = (...args) => getActiveBookingSystem()?.clearActiveBooking(...args);
const formatDateUK_UTC = (...args) => getActiveBookingSystem()?.formatDateUK_UTC(...args);
const isLakeAvailable = (...args) => getActiveBookingSystem()?.isLakeAvailable(...args);
const getAvailableSpots = (...args) => getActiveBookingSystem()?.getAvailableSpots(...args);
const getAllActiveBookingsByLake = (...args) => getActiveBookingSystem()?.getAllActiveBookingsByLake(...args);
const formatBookingForDisplay = (...args) => getActiveBookingSystem()?.formatBookingForDisplay(...args);
const clearIfExpired = (...args) => getActiveBookingSystem()?.clearIfExpired(...args);
...
function initializeBookingSystem() {
  ...
  const system = getActiveBookingSystem();
  if (system && system.cleanupExpiredBookings) {
    const cleaned = system.cleanupExpiredBookings();
    if (cleaned > 0) {
...
// =============================================================================
// TEST BOOKING CREATION (FOR DEMONSTRATION)
// =============================================================================

/**
 * Create a test booking for demonstration purposes
 * This creates a booking that starts now and lasts 24 hours
 */
function createTestBooking() {
  ...
  if (existingBooking) {
    const confirmReplace = confirm(
      'You already have an active booking. Do you want to replace it with a test booking?'
    );
    if (!confirmReplace) return;
    clearActiveBooking(currentUser.email);
  }
  
  // Create test booking starting now
  const startUtc = startOfLocalDayAsUTC(dateStr);
  const endUtc = startUtc + (24 * 60 * 60 * 1000); // 24 hours later
  ...
  const testBooking = {
    ...
    endUtc: endUtc,
    bookedOnUtc: now,
    notes: 'Test booking created for demonstration'
  };
  
  // Save the booking
  setActiveBooking(testBooking);
  
  // Also save to legacy format
  saveLegacyBooking(testBooking);
  
  console.log('✅ Test booking created:', testBooking);
  alert(`✅ Test booking created successfully!

Lake: ${testBooking.lakeName}
Session starts: ${new Date(testBooking.startUtc).toLocaleString()}
Duration: 24 hours

The booking card should now appear in your user dropdown menu!
Refresh the page to see it.`);
  
  // Reload to show the booking
  window.location.reload();
}

/**
 * Create a test booking that's currently active (started 2 hours ago)
 */
function createActiveTestBooking() {
  const currentUserData = localStorage.getItem('currentUser');
  if (!currentUserData) {
    alert('Please log in first to create a test booking');
    return;
  ...
  const twentyTwoHoursLater = twoHoursAgo + (24 * 60 * 60 * 1000); // 24 hours from start
  ...
  const testBooking = {
    ...
    endUtc: twentyTwoHoursLater,
    bookedOnUtc: twoHoursAgo - 1000,
    notes: 'Active test booking (started 2 hours ago)'
  };
  ...
  console.log('✅ Active test booking created:', testBooking);
  alert(`✅ Active test booking created!

Lake: ${testBooking.lakeName}
Status: ACTIVE NOW (started 2 hours ago)
...
/**
 * Create a test booking that's upcoming (starts tomorrow)
 */
function createUpcomingTestBooking() {
  const currentUserData = localStorage.getItem('currentUser');
  if (!currentUserData) {
    alert('Please log in first to create a test booking');
    return;
  ...
  const testBooking = {
    ...
    endUtc: endUtc,
    bookedOnUtc: now,
    notes: 'Upcoming test booking (starts tomorrow)'
  };
  ...
  console.log('✅ Upcoming test booking created:', testBooking);
  alert(`✅ Upcoming test booking created!

Lake: ${testBooking.lakeName}
Status: UPCOMING (starts tomorrow)
Start date: ${new Date(testBooking.startUtc).toLocaleDateString()}
...
function clearMyBooking() {
  ...
  if (!booking) {
    alert('You have no active booking to clear');
    return;
  }
  
  const confirmClear = confirm(`Clear your booking at ${booking.lakeName}?`);
  ...
  alert('✅ Your booking has been cleared');
  window.location.reload();
}

// Export test functions to window
window.createTestBooking = createTestBooking;
window.createActiveTestBooking = createActiveTestBooking;
window.createUpcomingTestBooking = createUpcomingTestBooking;
window.clearMyBooking = clearMyBooking;

console.log('📝 Test booking functions available:');
console.log('  - createTestBooking() - Create a test booking starting today');
console.log('  - createActiveTestBooking() - Create an active booking (started 2 hours ago)');
console.log('  - createUpcomingTestBooking() - Create an upcoming booking (tomorrow)');
console.log('  - clearMyBooking() - Clear your current booking');
