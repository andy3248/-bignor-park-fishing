...
function setupMonthSelector() {
    ...
    if (prevBtn) {
        prevBtn.addEventListener('click', () => {
            const activeIndex = Array.from(monthButtons).findIndex(btn => btn.classList.contains('active'));
            if (activeIndex > 0) {
                monthButtons[activeIndex - 1].click();
            }
        });
    }
    ...
    if (nextBtn) {
        nextBtn.addEventListener('click', () => {
            const activeIndex = Array.from(monthButtons).findIndex(btn => btn.classList.contains('active'));
            if (activeIndex < monthButtons.length - 1) {
                monthButtons[activeIndex + 1].click();
            }
        });
    }
...
function updateCalendar() {
    ...
    for (let i = 0; i < 42; i++) {
        ...
        dayElement.setAttribute('aria-label', currentDate.toDateString());
        ...
        if (currentDate.getMonth() !== currentMonth) {
            ...
            if (isAvailable) {
                ...
                dayElement.addEventListener('keydown', (e) => {
                    handleCalendarKeydown(e, dayElement, i);
                });
            } else {
...
function handleCalendarKeydown(e, currentElement, currentIndex) {
    const calendarDays = document.getElementById('calendarDays');
    const allDays = Array.from(calendarDays.children);
    let targetIndex = -1;
    ...
    if (targetIndex >= 0 && targetIndex < allDays.length) {
        ...
        if (targetDay && targetDay.getAttribute('tabindex') !== '-1') {
            targetDay.focus();
        }
    }
}
...
function selectDate(date) {
    ...
    const hiddenDateInput = document.getElementById('booking-date');
    if (hiddenDateInput) {
        hiddenDateInput.valueAsDate = selectedDate;
    }
...
function setupEventListeners() {
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            // If switching to Active Booking tab, refresh the card
            if (targetTab === 'active' && window.renderActiveBookingCard) {
                setTimeout(() => {
                    window.renderActiveBookingCard();
                }, 100);
            }
        });
    });
