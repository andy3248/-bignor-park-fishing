...
function updateLakeAvailability(dateString) {
    ...
    LAKES.forEach(lake => {
        ...
        const statusEl = document.getElementById(`${lake.slug}Status`);
        const slotsEl = document.getElementById(`${lake.slug}Slots`);
        const btnEl = document.getElementById(`book${lake.slug.charAt(0).toUpperCase() + lake.slug.slice(1)}Btn`);
        
        if (!statusEl || !slotsEl || !btnEl) {
            console.warn(`Elements not found for lake: ${lake.slug}`);
            return;
        }
...
function setupEventListeners() {
    ...
    const bookBignorBtn = document.getElementById('bookBignorBtn');
    const bookWoodBtn = document.getElementById('bookWoodBtn');
    
    if (bookBignorBtn) {
        bookBignorBtn.addEventListener('click', () => bookLake('bignor'));
    }
    
    if (bookWoodBtn) {
        bookWoodBtn.addEventListener('click', () => bookLake('wood'));
    }
